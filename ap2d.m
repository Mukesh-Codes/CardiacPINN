...
gathert= 4; % number of iterations at which V is outputted
...
Vsav=zeros(ncells,ncells,ceil((tend-tstar)/(dt*gathert))); % array where V will be saved during simulation
Wsav=zeros(ncells,ncells,ceil((tend-tstar)/(dt*gathert))); % array where W will be saved during simulation
